...
class ChatbotClient:
    ...
    async def get_context(self) -> Dict[str, Any]:
        ...
        """
        try:
            async with self.session.get(f"{self.base_url}/context/{self.session_id}") as response:
                return {
                    'status_code': response.status,
                    'data': await response.json() if response.status == 200 else None,
                    'error': None
    ...
    async def clear_context(self) -> Dict[str, Any]:
        ...
        """
        try:
            async with self.session.delete(f"{self.base_url}/context/{self.session_id}") as response:
                return {
                    'status_code': response.status,
                    'data': await response.json() if response.status == 200 else None,
                    'error': None
...
async def test_scenarios(client: ChatbotClient):
    ...
    avg_response_time = sum(r['response_time'] for r in results if r['success']) / max(1, successful_tests)
    ...
    print(f"Correct Intents: {correct_intents}/{len(results)} ({correct_intents/len(results)*100:.1f}%)")
...
async def main():
    ...
    parser.add_argument('--url', default='http://localhost:8000', help='Chatbot API URL')
    ...
    parser.add_argument('--simple-test', action='store_true', help='Run simple connectivity test')
