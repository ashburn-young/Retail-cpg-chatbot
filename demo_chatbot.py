    ...
    class DemoNLU:
        def __init__(self):
            ...
            self.intent_patterns = {
                "track_order": [
                    "track", "order", "status", "where is", "shipped", "delivery"
                ],
                "product_info": [
                    "tell me about", "information", "specs", "specifications", 
                    "features", "iphone", "macbook", "samsung", "product"
                ],
                "inventory_check": [
                    "in stock", "available", "availability", "inventory", 
                    "do you have", "is there"
                ],
                "store_locator": [
                    "store", "location", "near me", "address", "hours", 
                    "phone", "contact"
                ],
                "complaint": [
                    "complain", "problem", "issue", "not satisfied", 
                ...
                "return_refund": [
                    "return", "refund", "exchange", "money back", "replace"
                ]
            }
        ...
        def extract_entities(self, text):
            ...
            if order_matches:
                entities['order_number'] = [match[0] or match[1] for match in order_matches]
            
            # Extract product names
            products = []
            for ent in doc.ents:
                if ent.label_ in ["PRODUCT", "ORG"] and any(brand in ent.text.lower() 
        ...
        def generate_response(self, intent, entities=None, confidence=0.8):
            ...
            if entities:
                if 'order_number' in entities:
                    response = response.replace("{order_number}", entities['order_number'][0])
                ...
                if 'products' in entities:
                    ...
                    response = response.replace("{features}", "advanced technology and premium build quality")
                    response = response.replace("{price}", "999")
                
                if 'locations' in entities:
                    response = response.replace("{address}", f"123 Main St, {entities['locations'][0]}")
