...
class TestAPI:
    ...
    def test_chat_endpoint_authorized(self, client):
        ...
        response = client.post(
            "/chat",
            json={"message": "I want to track my order AB12345678"},
            headers=headers,
        )
...
class TestNLU:
    ...
    async def test_confidence_scoring(self, nlu_processor):
        ...
        low_confidence_messages = ["hello", "maybe", "xyz abc def"]
...
class TestResponseGeneration:
    ...
    async def test_escalation_logic(self, response_generator):
        ...
        complaint_response = await response_generator.generate_response(
            intent="complaint", entities={}, context={}, backend_data={}
        )
...
class TestAnalytics:
    ...
    async def test_error_logging(self, analytics_logger):
        ...
        await analytics_logger.log_error(
            session_id="test_session", error="Test error", message="Test message"
        )
...
class TestFullIntegration:
    ...
    async def test_complete_conversation_flow(self, client):
        ...
        response1 = client.post(
            "/chat", json={"message": "Hello, I need help"}, headers=headers
        )
...
class TestPerformance:
    ...
    async def test_concurrent_requests(self, client):
        ...
        async def make_request(i):
            response = client.post(
                "/chat", json={"message": f"Test message {i}"}, headers=headers
            )
...
class TestConfiguration:
    ...
    def test_settings_validation(self):
        ...
        valid_settings = Settings(
            ENVIRONMENT="test", CONFIDENCE_THRESHOLD=0.7, LOG_LEVEL="INFO"
        )
