...
class Settings(BaseSettings):
    ...
    APP_NAME: str = Field(
        default="Retail & CPG Customer Service Chatbot", description="Application name"
    )
    ...
    USE_MOCK_SERVICES: bool = Field(
        default=False, description="Use mock services for development"
    )
    ...
    DEV_CORS_ALL_ORIGINS: bool = Field(
        default=False, description="Enable CORS for all origins in development"
    )
    ...
    NLU_MODEL_PATH: str = Field(
        default="en_core_web_sm", description="spaCy model for NLU processing"
    )

    CONFIDENCE_THRESHOLD: float = Field(
        default=0.7, description="Minimum confidence threshold for automated responses"
    )

    MAX_MESSAGE_LENGTH: int = Field(
        default=1000, description="Maximum length of customer messages"
    )
    ...
    ORDER_API_KEY: Optional[str] = Field(
        default=None, description="API key for order management system"
    )
    ...
    INVENTORY_API_KEY: Optional[str] = Field(
        default=None, description="API key for inventory management system"
    )
    ...
    PRODUCT_API_KEY: Optional[str] = Field(
        default=None, description="API key for product information system"
    )
    ...
    STORE_API_KEY: Optional[str] = Field(
        default=None, description="API key for store locator system"
    )

    # API timeout settings
    API_TIMEOUT_SECONDS: int = Field(
        default=10, description="Timeout for backend API calls"
    )

    API_RETRY_COUNT: int = Field(
        default=3, description="Number of retries for failed API calls"
    )
    ...
    CONTEXT_STORAGE_TYPE: str = Field(
        default="memory", description="Context storage type: memory, redis, database"
    )

    CONTEXT_TTL_MINUTES: int = Field(
        default=30, description="Context time-to-live in minutes"
    )
    ...
    LOG_FILE_PATH: str = Field(
        default="logs/chatbot.log", description="Path to log file"
    )

    ANALYTICS_ENABLED: bool = Field(
        default=True, description="Enable analytics logging"
    )

    ANALYTICS_STORAGE_TYPE: str = Field(
        default="file", description="Analytics storage: file, database, cloud"
    )

    ANALYTICS_FILE_PATH: str = Field(
        default="data/analytics.jsonl", description="Path to analytics file"
    )
    ...
    COMPANY_NAME: str = Field(
        default="Your Retail Company", description="Company name for branding"
    )

    COMPANY_WEBSITE: str = Field(
        default="https://www.yourcompany.com", description="Company website URL"
    )

    SUPPORT_EMAIL: str = Field(
        default="support@yourcompany.com", description="Support email address"
    )

    SUPPORT_PHONE: str = Field(
        default="1-800-YOUR-HELP", description="Support phone number"
    )

    BRAND_COLOR: str = Field(
        default="#007bff", description="Primary brand color (hex code)"
    )
