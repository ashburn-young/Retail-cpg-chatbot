...
class AnalyticsLogger:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if storage_type == "file":
                ...
                logger.warning(
                    f"Unknown storage type {storage_type}, using file storage"
                )
    ...
    async def _calculate_summary_metrics(
        self, interactions: List[InteractionLog], hours: int
    ) -> Dict[str, Any]:
        ...
        escalation_rate = (
            escalations / total_interactions if total_interactions > 0 else 0
        )
        ...
        # Session statistics
        unique_sessions = len(
            set(interaction.session_id for interaction in interactions)
        )
        ...
        return {
            ...
            "average_response_time": (
                round(avg_response_time, 3) if avg_response_time else None
            ),
            "top_intents": [
    ...
    async def get_intent_analytics(
        ...
        try:
            ...
            low_confidence_count = sum(
                1 for c in confidences if c < low_confidence_threshold
            )
            ...
            return {
                ...
                "improvement_potential": (
                    round(low_confidence_count / total_count, 3)
                    if total_count > 0
                    else 0
                ),
            }
    ...
    async def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requests_per_minute": round(
                self.request_count / max(uptime.total_seconds() / 60, 1), 2
            ),
