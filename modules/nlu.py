...
class NLUProcessor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Could not load {self.settings.NLU_MODEL_PATH}, using en_core_web_sm"
                )
    ...
    def _setup_patterns(self):
        ...
        try:
            ...
            order_patterns = [
                [
                    {"TEXT": {"REGEX": r"^[A-Z]{2}\d{8,12}$"}}
                ],  # Order format: AB12345678
                [
                    {"LOWER": "order"},
                    {"TEXT": {"REGEX": r"^\d{8,15}$"}},
                ],  # "order 123456789"
        ...
        for intent, patterns in self.intent_patterns.items():
            ...
            if pattern_score > 0:
                ...
                intent_scores[intent] = max(
                    current_score, pattern_score / len(patterns)
                )
    ...
    def _adjust_confidence(
        self, intent: str, message: str, base_confidence: float
    ) -> float:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "model_name": (
                self.nlp.meta.get("name", "unknown") if self.nlp else "not_loaded"
            ),
